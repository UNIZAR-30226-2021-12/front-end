...
gameResponseHandler = async () => {
	...
	if( await statusCode == 410 ){
		...
		await this.props.navigation.push("MenuPrincipal", { token: this.state.token, miId: this.state.miId, español: this.state.español});
			...
			clearInterval(this.timer1);
			console.log('REACT ERES UN HIJO DE PUTA')
			//this.salirHandler()
			await this.props.navigation.push("MenuPrincipal", { token: this.state.token, miId: this.state.miId, español: this.state.español});
	}else{
		data = await response.json();
		...
		if(!this.state.boleano&&this.state.miTurno==this.state.turn){
...
buscarPausa = async () => {
	...
	if( await statusCode != 200 ){
		...
		await this.setState({gamePaused: this.state.gamePaused})
		console.log(this.state.gamePaused)
		if(!this.state.gamePaused){
			alert('El juego está en curso')
...
pausa = async () => {
	...
	if( await statusCode != 200 ){
		...
		await this.setState({token: this.state.token})
		await this.setState({estado: 6})
	}
}	
	estados = async () => {
										...
										<View key={this.state.cambiaTurno[1]} style={[styles.container, { width: 372.3, height: 150, backgroundColor:this.state.turnoJugadores[1] }]}> 
											<View style={styles.containerScreen4}>
												...
												<View style={styles.containerScreen5}>
														...
														{(this.state.español && <Text style={styles.sizeText}>Cartas: {this.state.playerNumCards[1]}</Text>) || (
															<Text style={styles.sizeText}>Cards: {this.state.playerNumCards[1]}</Text>
														  )}
												</View>
											</View>
										</View>
										...
										<View key={this.state.cambiaTurno[2]} style={[styles.container, { width: 372.3, height: 150, backgroundColor:this.state.turnoJugadores[2] }]}> 
											<View style={styles.containerScreen4}>
												...
												<View style={styles.containerScreen5}>
														...
														{(this.state.español && <Text style={styles.sizeText}>Cartas: {this.state.playerNumCards[2]}</Text>) || (
															<Text style={styles.sizeText}>Cards: {this.state.playerNumCards[2]}</Text>
														  )}
												</View>
											</View>
										</View>
										...
										<View key={this.state.cambiaTurno[3]} style={[styles.container, { width: 372.3, height: 150, backgroundColor:this.state.turnoJugadores[3] }]}> 
											<View style={styles.containerScreen4}>
												...
												<View style={styles.containerScreen5}>
														...
														{(this.state.español && <Text style={styles.sizeText}>Cartas: {this.state.playerNumCards[3]}</Text>) || (
															<Text style={styles.sizeText}>Cards: {this.state.playerNumCards[3]}</Text>
														  )}
												</View>
											</View>
										</View>
									...
									<View key={this.state.restart} style={styles.containerTablero}>
										...
										<View style={styles.containerColor}>
											{this.state.showColor &&
												<>
												<Button title={(this.state.español && "Rojo") || "Red"} color="#f71313" onPress={() => this.setState({ cartaColor: 'R'})}/>
												<Button title={(this.state.español && "Azul") || "Blue"} color="#1385f7" onPress={() => this.setState({ cartaColor: 'B'})}/>
												<Button title={(this.state.español && "Amarillo") || "Yellow"} color="#f4f713" onPress={() => this.setState({ cartaColor: 'Y'})}/>
												<Button title={(this.state.español && "Verde") || "Green"} color="#47f713" onPress={() => this.setState({ cartaColor: 'G'})}/>
												</>
											}
										</View>
									</View>
						...
						<View key={this.state.cambiaTurno[0]} style={[styles.container, { top: 40, width: 240, height: 590, backgroundColor:this.state.turnoJugadores[0] }]}> 
							<View style={styles.containerPerfil}>
								...
								<View style={styles.containerInfo}>
										...
										{(this.state.español && <Text style={styles.sizeText}>Mis cartas: {this.state.playerNumCards[0]}</Text>) || (
											<Text style={styles.sizeText}>My cards: {this.state.playerNumCards[0]}</Text>
										  )}
								</View>
								<View style={styles.containerBotones}>
									<Button title={(this.state.español && "PONER CARTA") || "PLAY CARD"} style={styles.botones} color="#e1a81a" onPress={() => this.setState({ estado: 2 })}/>
									<Button title={(this.state.español && "ROBAR CARTA") || "DRAW CARD"} style={styles.botones} color="#e1a81a" onPress={() => this.setState({ estado: 3})}/>
									<Button title={(this.state.español && "PAUSA") || "PAUSE"} style={styles.botones} color="#e1a81a" onPress={() => this.setState({ estado: 7})}/>
									<Button title="UNOZAR" style={styles.botones} color="#00ff00" onPress={() => this.setState({ hasSaidUnozar: true })}/>
									<Button title={(this.state.español && "SALIR") || "QUIT"} style={styles.botones} color="#ff0000" onPress={() => this.setState({ estado: 5})}/>
								</View>
							</View>
						</View>
					</View>
