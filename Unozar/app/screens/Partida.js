...
class Partida extends React.Component {
  ...
  playCardHandler = async () => {
    ...
    await console.log(
      "PONER CARTA: " + this.state.playerCards[this.state.carta]
    );
    if ((await this.state.playerCards[this.state.carta]) == "XXC") {
    } else if ((await this.state.playerCards[this.state.carta]) == "XX4") {
    } else {
      await this.setState({
        cartaColor: this.state.playerCards[this.state.carta].slice(1, 2),
      });
    }
    ...
    let data;
    const response = await fetch(
      "https://unozar.herokuapp.com/game/playCard",
      requestOptions
    );
    data = await response.json();
  ...
  robarCardHandler = async () => {
    ...
    let data;
    const response = await fetch(
      "https://unozar.herokuapp.com/game/draw",
      requestOptions
    );
    data = await response.json();
    let statusCode = response.status;
    if ((await statusCode) != 200) {
      ...
      await this.setState({ token: data.token });
      await this.setState({
        numCartasJugador1: this.state.numCartasJugador1 + 1,
      });
  ...
  gameResponseHandler = async () => {
    ...
    let data;
    const response = await fetch(
      "https://unozar.herokuapp.com/game/readGame",
      requestOptions
    );
    let statusCode = await response.status;
    if ((await statusCode) == 410) {
      ...
      if (
        (await this.state.turn) != data.turn ||
        (await this.state.topDiscard) != data.topDiscard
      ) {
        for (let i = 0; i < this.state.maxPlayers; i++) {
        ...
        for (let i = 0; i < this.state.maxPlayers; i++) {
          await console.log(
            "NUMERO DE CARTAS DE " +
              i +
              ": " +
              data.playersNumCards[this.state.turnos[i]]
          );
          this.state.playerNumCards[i] = await data.playersNumCards[
            this.state.turnos[i]
          ];
      ...
      await console.log(
        "readGameResponse [" + "READ TOKEN: " + this.state.token,
        this.state.maxPlayers,
        data.topDiscard,
        this.state.playerCards,
        this.state.turn,
        this.state.playersIds,
        this.state.playerNumCards,
  ...
  pausa = async () => {
    ...
    let data;
    const response = await fetch(
      "https://unozar.herokuapp.com/game/pause",
      requestOptions
    );
    data = await response.json();
    let statusCode = response.status;
  ...
  seleccionarCarta(i) {
    ...
    if (
      this.state.playerCards[i] == "XXC" ||
      this.state.playerCards[i] == "XX4"
    ) {
