...
class PerfilAmigos extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      body:
        nombreJugador1 + " desea invitarle a una partida de Unozar: " + gameId,
      cc: "",
      bcc: "",
  ...
  deleteAmigo = async () => {
    ...
    let statusCode;
    response = await fetch(
      "https://unozar.herokuapp.com/player/deleteFriend",
      requestOptions
    );
    data = await response.json();
    statusCode = response.status;
    if ((await statusCode) != 200) {
      ...
      await this.props.navigation.push("Amigos", {
        token: this.state.token,
        playerId: this.state.playerId,
        gameId: this.state.gameId,
        invitar: true,
          ...
          <View style={styles.screen}>
            <View style={styles.formContainer}>
              ...
              <Text style={styles.texto}>
                {" "}
                {this.state.alias} {this.state.email}{" "}
              </Text>
              {!this.state.amigo && (
                <Text style={styles.texto}> {this.state.playerId} </Text>
              )}
              {this.state.amigo && (
                <Text style={styles.texto}>
                  {" "}
                  {this.state.idJugadorInvitar}{" "}
                </Text>
              )}
              <Text style={styles.texto}> Partidas públicas </Text>
              <Text style={styles.texto}>
                {" "}
                J: {this.state.publicTotal} W: {this.state.publicWins}
              </Text>
              <Text style={styles.texto}> Partidas privadas </Text>
              <Text style={styles.texto}>
                {" "}
                J: {this.state.privateTotal} W: {this.state.privateWins}
              </Text>
              {this.state.invitar && (
