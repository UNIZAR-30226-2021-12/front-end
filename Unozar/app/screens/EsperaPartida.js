...
class EsperaPartida extends React.Component {
  ...
  readHandler = async () => {
    ...
    if ((await statusCode) != 200) {
      ...
      await console.log(
        ...
        this.state.playerId3,
        this.state.playerId4 + "]"
      );
    }
  };
  readPlayerHandler = async () => {
    ...
    if (
      ...
      this.state.playerId1 != "EMPTY" &&
      this.state.playerId1 != "BOT" &&
    ...
    if (
      this.state.maxPlayers >= 2 &&
      this.state.playerId2 != "EMPTY" &&
      this.state.playerId2 != "BOT" &&
    ...
    if (
      ...
      this.state.playerId3 != "EMPTY" &&
      this.state.playerId3 != "BOT" &&
            ...
            <View style={styles.containerParticipantes}>
              {this.state.miId != this.state.playerId1 && (
                <Text style={styles.textoTitulo}>
                  {this.state.CustomTextLocal.esperandoACreador1}{" "}
                  {this.state.nombreJugador1}{" "}
                  {this.state.CustomTextLocal.esperandoACreador2}
                </Text>
              )}
              {this.state.miId == this.state.playerId1 && (
                <Text style={styles.textoTitulo}>
                  {this.state.CustomTextLocal.faltanJugadores1}{" "}
                  {this.state.maxPlayers -
                    this.state.numBots -
                    this.state.jugadores}{" "}
                  {this.state.CustomTextLocal.faltanJugadores2}
                </Text>
              )}
              <Text style={styles.textoId}>
                {" "}
                {this.state.CustomTextLocal.jugador1}:{" "}
                {this.state.nombreJugador1}{" "}
              </Text>
              {this.state.maxPlayers - this.state.numBots >= 2 && (
                <Text style={styles.textoId}>
                  {" "}
                  {this.state.CustomTextLocal.jugador2}:{" "}
                  {this.state.nombreJugador2}{" "}
                </Text>
              )}
              {this.state.maxPlayers - this.state.numBots >= 3 && (
                <Text style={styles.textoId}>
                  {" "}
                  {this.state.CustomTextLocal.jugador3}:{" "}
                  {this.state.nombreJugador3}{" "}
                </Text>
              )}
              {this.state.maxPlayers - this.state.numBots == 4 && (
                <Text style={styles.textoId}>
                  {" "}
                  {this.state.CustomTextLocal.jugador4}:{" "}
                  {this.state.nombreJugador4}{" "}
                </Text>
              )}
              {this.state.miId == this.state.playerId1 &&
                this.state.jugadores == this.state.maxPlayers && (
                  <Button
                    title={this.state.CustomTextLocal.empezarPartida}
                    color="#40d81b"
              ...
              <View style={styles.boton}>
                {this.state.jugadores != this.maxPlayers && (
                  <Button
                    title={this.state.CustomTextLocal.invitarAmigo}
                    onPress={() => this.setState({ estado: 4 })}
                  />
                )}
                <Button
                  title={this.state.CustomTextLocal.salir}
